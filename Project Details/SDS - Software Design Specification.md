...
Logic:
  ...
  2. Switch on provider:
     ...
     - Ollama → LLM(base_url="http://localhost:11434", model="ollama/{model}")
...
|------|-------------|----------|-----------|
| API Keys | Environment variables (in-memory) | `os.environ` | Session-scoped in practice; generally cleared on browser close because they are set as `os.environ` variables that persist for the Streamlit server process lifetime |
