...
| **Condition** | Web search is disabled when using Ollama provider because the Exa API key input is hidden when Ollama is selected, so web search will fail if the agent attempts it, though the tool is still attached to the agent |
| **Priority** | High |
...
- **Authentication:** Exa API uses `x-api-key` header; OpenAI and GROQ authentication is handled internally by the CrewAI `LLM` abstraction using the provided API key
